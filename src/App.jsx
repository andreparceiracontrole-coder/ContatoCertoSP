...
function RegisterForm({mode,onSubmit}){
  ...
  return <div className="space-y-2">
    ...
    {mode==="montador" && <>
      <div className="bg-yellow-50 border-2 border-yellow-400 p-3 rounded-2xl">
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <input type="file" accept="image/*" onChange={handleFoto} className="w-full text-xs p-2 border-2 border-dashed rounded-xl bg-white"/>
            <div className="text-[10px] text-gray-500 mt-1">📱 Galeria do celular - JPG/PNG até 5MB - Será compactada para 300x300</div>
      ...
      <div className="flex gap-2"><input placeholder="Cidade atendida até 3" value={ci} onChange={e=>setCi(e.target.value)} className="flex-1 border rounded-xl p-3"/><button type="button" onClick={()=>{ if(f.cidades.length<3 && ci){ setF({...f,cidades:[...f.cidades,ci]}); setCi(""); } }} className="bg-gray-100 px-4 rounded-xl">+</button></div>
    ...
    <button onClick={()=>{
      ...
      onSubmit(f);
    }} className="bg-[#FF7A00] text-white w-full py-4 rounded-2xl font-bold">Cadastrar no Supabase {mode==="montador" && !f.foto?"(FOTO OBRIGATÓRIA)":""}</button>
  </div>;
}
